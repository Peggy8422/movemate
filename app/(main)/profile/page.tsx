...
const getProfileData = async () => {
  ...
  const answersLookup: Record<string, any> = {};
  for (const item of userAnswers) {
    answersLookup[item.questionTitle] = item;
  }

  // calculate age
  const birthDate = new Date(answersLookup["生日"]?.birthDate);
  ...
  return {
    ...
    userSexual: answersLookup["性別"]?.selections[0].selectionText,
    userAge: age,
    userHeight: answersLookup["身高"]?.textAnswers,
    userWeight: answersLookup["體重"]?.textAnswers,
    livingArea: answersLookup["住哪?"]?.textAnswers[0]?.slice(0, 3),
    userLevel: "1",
    selfIntroduction: userBasicInfo.intro || "",
    personalTags: userBasicInfo.personalTags || ["新增標籤"],
    preferance: {
      place: answersLookup["最常在哪裡運動？"],
      sportType: answersLookup["喜歡的運動種類？"],
    },
  };
};
